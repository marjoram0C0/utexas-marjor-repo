...
def step_size(s):
    ...
    return STEP_SIZE_CONSTANT - (hash_idx % STEP_SIZE_CONSTANT)
...
def is_reducible(s, hash_table, hash_memo):
    ...
    """
    # base case 1: word is in hash_memo
    if find_word(s, hash_memo):
        return True

    # base case 2: word is "a", "i", or "o"
    if s in ("a", "i", "o"):
        return True

    # choice 1 to n: remove 1st to nth letter
    for i in range(len(s)):
        if find_word(s[:i] + s[i+1:], hash_table) and \
        is_reducible(s[:i] + s[i+1:], hash_table, hash_memo):
...
def main():
    ...
    try:
        while cur_word:
            word_list.append(cur_word.strip())
            cur_word = input()
    except EOFError:
        pass # do nothing
    ...
    for word in word_list:
        if is_reducible(word, hash_list, hash_memo):
            reducible_words += [word]

    # find the largest reducible words in reducible_words
    result = get_longest_words(reducible_words)
