...
def group_sum(start, nums, target):
    ...
    # choice 1: we include the current start num in the sum
    if group_sum(start + 1, nums, target - nums[start]):
        return True
    # choice 2: we do not include the current sum
...
def group_no_adj(start, nums, target):
    ...
    # choice 1: we include the current start num in the sum
    # if we choose this, we skip the next value.
...
def group_sum_5(start, nums, target):
    ...
    # choice 1: we include the current start num in the sum
    # if the previous value is a multiple of 5, we can't choose 1s.
    if not (nums[start] == 1 and start != 0 and nums[start - 1] %  5 == 0):
        if group_sum_5(start + 1, nums, target - nums[start]):
            return True
    # choice 2: we do not include the current sum
    # multiples of 5 cannot be skipped
    if nums[start] % 5 != 0:
...
def group_sum_clump(start, nums, target):
    ...
    # choice 1: we include the current start num in the sum
    if group_sum_clump(start + i, nums, target - (nums[start] * i) ):
        return True
    # choice 2: we do not include the current sum
    return group_sum_clump(start + i, nums, target)
